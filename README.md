...
The scores below were generated by running the live API against the listed token mints on May 10, 2026. Each result is reproducible by anyone with a Helius API key by pointing the backend at the same mint address and recording the JSON response.

| Token | Mint Address | Score | Grade | Notes |
|-------|:------------:|:-----:|:-----:|-------|
| **USDC** | `EPjF…Dt1v` | 97 | A+ | Institutional stablecoin. Infrastructure-routed volume (CEX hot wallets, AMM programs) correctly excluded from concentration penalty by the Proof-of-Ecosystem tier classification. |
| **JTO** | `jtoj…7mCL` | 87 | A | Jito staking token. Distributed sender base, healthy buyer diversity, no circular flow patterns detected. |
| **BONK** | `DezX…B263` | 87 | A | Top-tier meme coin. Passes all three heuristics; infrastructure exclusions properly applied to exchange and AMM wallets. |
| **KIKI** | `7Usc…pump` | 0 | F | Pump.fun launch exhibiting textbook self-trading: the top sender wallet (`7hmG…iksH`) accounts for 57% of outbound volume **and** 56% of inbound volume — a wallet transacting with itself. Diversity index 0.26, zero DEX integration. |
| JUP | `JUPy…DvCN` | — | — | Sample size below the confidence threshold (41 transfers in the analysis window). The system declines to issue a grade rather than infer from insufficient data. |
| WIF | `EKpQ…zcjm` | — | — | Sample size below the confidence threshold (90 transfers in the analysis window). The system declines to issue a grade rather than infer from insufficient data. |

These results demonstrate three properties that a useful integrity tool must satisfy simultaneously:

- **Specificity** — legitimate high-volume tokens (USDC, JTO, BONK) are not falsely flagged as wash-traded despite concentrated volume profiles.
- **Sensitivity** — tokens with mathematically unambiguous self-trading patterns (KIKI: same wallet on both sides of 56%+ of volume) are correctly graded F.
- **Epistemic humility** — when the transaction sample is too sparse for reliable inference (JUP, WIF), the system refuses to grade rather than guess.
---
